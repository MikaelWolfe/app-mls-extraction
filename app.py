...
def normalize_address(addr):
    addr = addr.lower()
    addr = re.sub(r"[.,\-]", " ", addr)  # enlever . , -
    addr = re.sub(r"\s+", " ", addr).strip()  # enlever espaces multiples
    return addr
...
def extract_civic_numbers(addr):
    """
    Extrait tous les numéros civiques d'une adresse.
    Gère les plages type 4270-4280.
    """
    nums = []
...
def civic_in_mls(addr_other, addr_mls):
    """
    Vérifie si le numéro civique de addr_other se trouve dans la plage MLS.
    """
...
with col2:
    ...
    if uploaded_pdfs_other:
        ...
        for idx, uploaded_pdf in enumerate(uploaded_pdfs_other, start=1):
            ...
            # Écriture dans Excel avec addition pour cas de plage civique
            if rangée_mls:
